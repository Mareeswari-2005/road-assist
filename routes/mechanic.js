...
// Reject request
router.put('/request/:id/reject', mechanicAuth, async (req, res) => {
  try {
    ...
    console.log('Rejecting request, notifying user:', request.userId._id.toString());
    ...
    if (io && request.userId) {
      ...
      io.to(userId).emit('request-rejected', {
        ...
        message: `${req.user.name} declined your request. Please select another mechanic.`
      });
      
      // Broadcast as fallback
      io.emit('request-rejected-broadcast', {
        forUserId: userId,
        requestId: request._id,
        mechanicName: req.user.name,
        message: `${req.user.name} declined your request. Please select another mechanic.`
...
router.get('/profile', mechanicAuth, async (req, res) => {
  try {
...
router.get('/availability-status', mechanicAuth, async (req, res) => {
  try {
    const shop = await Shop.findOne({ mechanicId: req.user._id });
    res.json({ isOnline: shop?.isOnline || false });
  } catch (error) {
    res.status(500).json({ message: 'Server error', error: error.message });
  }
});
